package com.stu.com.app2.custom_view.CircleTextProgressbar;
...
public class CircleTextProgressbar extends TextView {
    ...
    private int outLineColor = Color.BLACK; //外部轮廓的颜色。
    private int outLineWidth = 2;// 外部轮廓的宽度。
...
//    内部圆的颜色。
    ...
    protected void onDraw(Canvas canvas) {
        //获取view的边界 我们可以通过getDrawingRect(Rect)获取到绘制区域，通过绘制区域计算出这个区域可以绘制圆的半径。
...
//        圆心是绘制区域的圆心，半径是绘制区域圆的半径减去外部轮廓圆线的宽度的一半，这样刚好外部轮廓线和内部填充圆紧靠着。
        ...
        //画字  为了我们的绘制和TextView自身的绘制不重叠，我们干掉了super.onDraw(canvas);，所以这里我们要把TextView的字也要写上去。
...
//        在指定绘制区域，因为不能把外部轮廓线覆盖了，所以要贴近外部轮廓线的内部画，
// 所以要最外层绘制圆的区域，所以要减去(外部圆线的宽 + 进度条线的宽) / 2得出来的界线就是进度条的边界。
